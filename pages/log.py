...
##############################################################################################
    ...
    default_columns = ['IMG', 'ITEM_ID', 'SKU'  ,'TITLE',  'MSHOPS_PRICE', 'QUANTITY',  "STATUS", "URL", "ITEM_LINK"]
    ...
    st.dataframe(
        select_data, 
        column_config={
            "URL": st.column_config.LinkColumn(display_text="Link do Anúncio"),
            "ITEM_LINK": st.column_config.LinkColumn(display_text="Editar Anúncio"),
            "IMG": st.column_config.ImageColumn(
                "Preview", help="Streamlit app preview screenshots", width=130
            )
        }
    )
...
if "messages" not in st.session_state or st.button("Limpar histórico de conversas"):
